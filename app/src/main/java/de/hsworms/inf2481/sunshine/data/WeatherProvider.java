...
public class WeatherProvider extends ContentProvider {
    ...
    @Override
    public int bulkInsert(Uri xUri, ContentValues[] xValues) {
        final SQLiteDatabase db = mOpenHelper.getWritableDatabase();
        final int match = sUriMatcher.match(xUri);
        switch (match) {
            case WEATHER:
                db.beginTransaction();
                int returnCount = 0;
                try {
                    for (ContentValues value : xValues) {
                        normalizeDate(value);
                ...
                return returnCount;
            default:
                return super.bulkInsert(xUri, xValues);
        }
    }
